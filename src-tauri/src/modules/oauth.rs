...
// Google OAuth runtime credential resolver
const XOR_KEY: u8 = 0x5a;
const ENC_CID: &[u8] = &[
    107, 106, 109, 107, 106, 106, 108, 106, 108, 106, 111, 99, 107, 119, 46, 55, 50, 41, 41, 51,
    52, 104, 50, 104, 107, 54, 57, 40, 63, 104, 105, 111, 44, 46, 53, 54, 53, 48, 50, 110, 61,
    110, 106, 105, 63, 42, 116, 59, 42, 42, 41, 116, 61, 53, 53, 61, 54, 63, 47, 41, 63, 40, 57,
    53, 52, 46, 63, 52, 46, 116, 57, 53, 55,
];
const ENC_CSEC: &[u8] = &[
...
fn get_default_client_id() -> String {
    std::env::var("ANTIGRAVITY_CLIENT_ID")
        ...
        .unwrap_or_else(|| {
            let bytes: Vec<u8> = ENC_CID.iter().map(|&b| b ^ XOR_KEY).collect();
...
fn get_default_client_secret() -> String {
    std::env::var("ANTIGRAVITY_CLIENT_SECRET")
        ...
        .filter(|s| !s.trim().is_empty())
        .unwrap_or_else(|| {
            let bytes: Vec<u8> = ENC_CSEC.iter().map(|&b| b ^ XOR_KEY).collect();
            String::from_utf8(bytes).unwrap_or_default()
        })
}
