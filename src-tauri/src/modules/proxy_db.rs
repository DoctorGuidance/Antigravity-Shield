...
pub fn cleanup_old_logs(days: i64) -> Result<usize, String> {
    ...
    // Note: ProxyRequestLog.timestamp stores UTC timestamp in milliseconds
    ...
    // Only execute VACUUM when rows were actually deleted to prevent saturating disk on every startup
    if deleted > 0 {
