...
pub struct ProxyToken {
    ...
    pub device_profile: Option<crate::models::DeviceProfile>, // [NEW M1] Per-account isolated device fingerprint
}

/// 403 细分分类与安全隔离协议
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ForbiddenKind {
    /// 1. 验证挑战 (VALIDATION_REQUIRED)：提取 URL，施加 10 分钟验证冷却与内存隔离，不写盘注销
    ValidationRequired {
        validation_url: Option<String>,
        cooldown_secs: u64,
    },
    /// 2. 确认的永久违规/封号：仅在有确切证据时落盘写入 is_forbidden
    PermanentPolicyViolation {
        reason: String,
    },
    /// 3. 瞬时 403 (网络/WAF/边缘 IP 拦截)：实行 5 分钟内存隔离区冷却，保持磁盘文件完好
    TransientQuarantine {
        cooldown_secs: u64,
    },
}
...
/// 提取验证或申诉链接（如果存在）
pub fn extract_validation_url(error_text: &str) -> Option<String> {
    if let Ok(parsed_json) = serde_json::from_str::<serde_json::Value>(error_text) {
        if let Some(details) = parsed_json.pointer("/error/details").and_then(|v| v.as_array()) {
            for detail in details {
                if let Some(meta) = detail.get("metadata") {
                    if let Some(url) = meta.get("validation_url").and_then(|v| v.as_str()) {
                        return Some(url.to_string());
                    }
                    if let Some(url) = meta.get("appeal_url").and_then(|v| v.as_str()) {
                        return Some(url.to_string());
                    }
                }
            }
    ...
    url_regex.find(error_text).map(|m| m.as_str().replace("\\u0026", "&"))
}

/// 针对 403 Forbidden 错误文本进行三级细分判定
pub fn classify_forbidden_error(error_text: &str) -> ForbiddenKind {
    let lower = error_text.to_lowercase();

    // 1. 优先判定 VALIDATION_REQUIRED 挑战
    if error_text.contains("VALIDATION_REQUIRED")
        || lower.contains("verify your account")
        || error_text.contains("validation_url")
        || error_text.contains("appeal_url")
    {
        let url = extract_validation_url(error_text);
        return ForbiddenKind::ValidationRequired {
            validation_url: url,
    ...
    // 2. 判定确切的永久封号/违规
    if error_text.contains("CONSUMER_SUSPENDED")
        || error_text.contains("ACCOUNT_DISABLED")
        || error_text.contains("USER_DISABLED")
        || error_text.contains("POLICY_VIOLATION")
        || error_text.contains("TERMS_OF_SERVICE_VIOLATION")
        || lower.contains("account has been disabled")
        || lower.contains("account disabled")
        || lower.contains("has been suspended")
        || lower.contains("violates google's terms of service")
        || lower.contains("violates google terms of service")
        || (lower.contains("terms of service") && (lower.contains("violation") || lower.contains("suspended")))
    {
        return ForbiddenKind::PermanentPolicyViolation {
            reason: error_text.to_string(),
        };
    }

    // 3. 其它所有 403 均视为可恢复瞬态错误，施加 5 分钟内存隔离
    ForbiddenKind::TransientQuarantine {
...
pub struct QuarantineRecord {
    pub quarantined_at: i64,      // 隔离开始时间戳（秒）
    pub quarantine_until: i64,    // 隔离解除时间戳（秒）
    pub reason: String,
...
pub struct TokenManager {
    ...
    quarantine_records: Arc<DashMap<String, QuarantineRecord>>, // [NEW M1] 内存安全隔离区
    ...
    quota_protection_cache: Arc<std::sync::RwLock<Option<(bool, std::time::Instant)>>>,
}
...
impl Clone for TokenManager {
    fn clone(&self) -> Self {
        Self {
            ...
            last_used_account: Arc::clone(&self.last_used_account),
            data_dir: self.data_dir.clone(),
            rate_limit_tracker: Arc::clone(&self.rate_limit_tracker),
            quarantine_records: Arc::clone(&self.quarantine_records),
            sticky_config: Arc::clone(&self.sticky_config),
            session_accounts: Arc::clone(&self.session_accounts),
            preferred_account_id: Arc::clone(&self.preferred_account_id),
            health_scores: Arc::clone(&self.health_scores),
            circuit_breaker_config: Arc::clone(&self.circuit_breaker_config),
            refresh_locks: Arc::clone(&self.refresh_locks),
            load_code_assist_inflight: Arc::clone(&self.load_code_assist_inflight),
            invalid_grant_failures: Arc::clone(&self.invalid_grant_failures),
            auto_cleanup_handle: Arc::clone(&self.auto_cleanup_handle),
            cancel_token: self.cancel_token.clone(),
            image_scheduler: std::sync::RwLock::new(
                self.image_scheduler
                    .read()
                    .ok()
                    .and_then(|g| g.clone()),
            ),
            quota_protection_cache: Arc::clone(&self.quota_protection_cache),
        }
    }
}
...
impl TokenManager {
    ...
    pub fn new(data_dir: PathBuf) -> Self {
        Self {
            ...
            quarantine_records: Arc::new(DashMap::new()), // [NEW M1]
            ...
            quota_protection_cache: Arc::new(std::sync::RwLock::new(None)),
        }
    }
    ...
    pub fn is_quota_protection_enabled(&self) -> bool {
        if let Ok(guard) = self.quota_protection_cache.read() {
            if let Some((enabled, timestamp)) = *guard {
        ...
        let enabled = crate::modules::config::load_app_config()
            .map(|cfg| cfg.quota_protection.enabled)
            .unwrap_or(false);

        if let Ok(mut guard) = self.quota_protection_cache.write() {
    ...
    /// 启动限流记录与内存隔离区自动清理后台任务（每15秒检查并清除过期记录）
    pub async fn start_auto_cleanup(&self) {
        let tracker = self.rate_limit_tracker.clone();
        let quarantine_records = self.quarantine_records.clone();
            ...
            loop {
                tokio::select! {
                    ...
                    _ = interval.tick() => {
                        ...
                        quarantine_records.retain(|account_id, record| {
                            if now >= record.quarantine_until {
                                tracing::info!(
                                    "🛡️ [Quarantine] Account {} quarantine cooldown expired, restored to pool",
                                    account_id
                                );
                                false
                            } else {
    ...
    async fn get_token_filtered(
        ...
        // [FIX / Issue #3348] 异步离线处理待重新加载的账号，避免阻塞热路径
        ...
        if !pending_reload.is_empty() {
            ...
            tokio::spawn(async move {
                for account_id in pending_reload {
                    if let Err(e) = this.reload_account(&account_id).await {
        ...
        // 【优化 Issue #284, #3348】扩展超时至 15 秒，防止高并发下超时
        ...
        .await
        {
            Ok(result) => result,
            Err(_) => Err(
                "Token acquisition timeout (15s) - system too busy or deadlock detected".to_string(),
            ),
        }
    }
    ...
    async fn get_token_internal(
        ...
        tokens_snapshot.retain(|token| {
            !excluded_accounts.contains(&token.account_id)
                && !self.is_quarantined(&token.account_id)
        });
        ...
        // 【新增 / Issue #3348】检查配额保护是否启用（优先从内存缓存读取，避免热路径读盘）
        let quota_protection_enabled = self.is_quota_protection_enabled();
                    ...
                    if last_time.elapsed().as_secs() < 60 && !attempted.contains(account_id) {
                            ...
                            tokens_snapshot.iter().find(|t| &t.account_id == account_id)
                        {
                            ...
                            if !self
                                ...
                                .await
                                && !self.is_quarantined(&found.account_id)
                                && !(quota_protection_enabled
    ...
    pub async fn fetch_and_lock_with_realtime_quota(
        ...
        match crate::modules::quota::fetch_quota(&access_token, email, Some(&account_id)).await {
            Ok((quota_data, _project_id)) => {
                // [HARDENED M1] 429 触发实时刷新时，彻底隔离 is_forbidden 状态，防止级联封禁
                if quota_data.is_forbidden {
                    tracing::warn!(
                        "账号 {} 实时配额刷新返回 is_forbidden，忽略此标记以防止 429 级联封禁",
                        email
                    );
                    return false;
                }
    ...
    /// [NEW #3384] 获取当前优先/指定账号（如果配置了固定账号模式）
    pub async fn get_preferred_account_id(&self) -> Option<String> {
        self.preferred_account_id.read().await.clone()
    }

    /// [NEW #3384] 获取指定账号所支持的所有模型集合
    pub fn get_account_collected_models(&self, account_id: &str) -> Option<std::collections::HashSet<String>> {
    ...
    /// [NEW M1] 将账号放入内存隔离区（不写盘，冷却时间到期后自动恢复）
    pub async fn quarantine_account(
        ...
        let now = chrono::Utc::now().timestamp();
        let quarantine_until = now + cooldown_secs as i64;

        self.quarantine_records.insert(
        ...
        tracing::warn!(
            "🛡️ [Quarantine] Account {} placed in memory quarantine for {}s (until {}), reason: {}",
            account_id,
            cooldown_secs,
            chrono::DateTime::from_timestamp(quarantine_until, 0)
                .map(|dt| dt.format("%H:%M:%S").to_string())
                .unwrap_or_else(|| quarantine_until.to_string()),
            truncate_reason(reason, 200)
        );
    }

    /// [NEW M1] 检查账号是否处于活跃的内存隔离期
    pub fn is_quarantined(&self, account_id: &str) -> bool {
        if let Some(record) = self.quarantine_records.get(account_id) {
            let now = chrono::Utc::now().timestamp();
            if now < record.quarantine_until {
                return true;
            }
    ...
    /// [NEW M1] 清理过期的隔离区记录
    pub fn cleanup_expired_quarantine(&self) {
        let now = chrono::Utc::now().timestamp();
        self.quarantine_records.retain(|account_id, record| {
            if now >= record.quarantine_until {
                tracing::info!(
                    "🛡️ [Quarantine] Account {} quarantine cooldown expired, restored to pool",
                    account_id
    ...
    /// [NEW M1] 获取账号剩余隔离时间（秒）
    pub fn get_quarantine_remaining_secs(&self, account_id: &str) -> Option<u64> {
        self.quarantine_records.get(account_id).and_then(|record| {
            let now = chrono::Utc::now().timestamp();
            if record.quarantine_until > now {
                Some((record.quarantine_until - now) as u64)
            } else {
                None
            }
        })
...
mod tests {
    ...
    #[test]
    fn test_classify_forbidden_validation_required() {
        let err1 = r#"{"error":{"code":403,"message":"VALIDATION_REQUIRED: verify your account at https://accounts.google.com/challenge","status":"PERMISSION_DENIED"}}"#;
        let kind1 = classify_forbidden_error(err1);
        assert!(matches!(kind1, ForbiddenKind::ValidationRequired { .. }));
        if let ForbiddenKind::ValidationRequired { validation_url, cooldown_secs } = kind1 {
            assert_eq!(cooldown_secs, 600);
            assert_eq!(validation_url, Some("https://accounts.google.com/challenge".to_string()));
        }

        let err2 = "Please verify your account to continue";
        let kind2 = classify_forbidden_error(err2);
        assert!(matches!(kind2, ForbiddenKind::ValidationRequired { .. }));
    }

    #[test]
    fn test_classify_forbidden_permanent_policy_violation() {
        let err1 = "Account CONSUMER_SUSPENDED: violates terms of service";
        let kind1 = classify_forbidden_error(err1);
        assert!(matches!(kind1, ForbiddenKind::PermanentPolicyViolation { .. }));

        let err2 = "User account has been disabled by administrator";
        let kind2 = classify_forbidden_error(err2);
        assert!(matches!(kind2, ForbiddenKind::PermanentPolicyViolation { .. }));
    }

    #[test]
    fn test_classify_forbidden_transient_quarantine() {
        let err1 = "403 Forbidden: Edge proxy blocked request";
        let kind1 = classify_forbidden_error(err1);
        assert_eq!(kind1, ForbiddenKind::TransientQuarantine { cooldown_secs: 300 });

        let err2 = "Access denied";
        let kind2 = classify_forbidden_error(err2);
        assert_eq!(kind2, ForbiddenKind::TransientQuarantine { cooldown_secs: 300 });
    }

    #[tokio::test]
    async fn test_quarantine_account_lifecycle() {
        let tmp_root = std::env::temp_dir().join(format!(
            "antigravity-quarantine-test-{}",
            uuid::Uuid::new_v4()
        ));
        let manager = TokenManager::new(tmp_root);

        assert!(!manager.is_quarantined("acc_test_1"));
        manager.quarantine_account("acc_test_1", 300, "WAF block", false).await;
        assert!(manager.is_quarantined("acc_test_1"));
        assert!(manager.get_quarantine_remaining_secs("acc_test_1").unwrap() > 0);

        // Simulated immediate expiration
        if let Some(mut rec) = manager.quarantine_records.get_mut("acc_test_1") {
            rec.quarantine_until = chrono::Utc::now().timestamp() - 1;
        }
        assert!(!manager.is_quarantined("acc_test_1"));
        manager.cleanup_expired_quarantine();
        assert!(!manager.quarantine_records.contains_key("acc_test_1"));
    }
