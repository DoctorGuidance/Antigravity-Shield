        ...
        if status_code == 403 {
            ...
            match forbidden_kind {
                crate::proxy::token_manager::ForbiddenKind::ValidationRequired { cooldown_secs, .. } => {
                    tracing::warn!(
                        "[Claude] VALIDATION_REQUIRED detected on account {}, applying {}s cooldown",
                        email,
                        cooldown_secs
                    );
                    ...
                    token_manager
                        .quarantine_account(&account_id, cooldown_secs, &error_text, true)
                        .await;
                }
                crate::proxy::token_manager::ForbiddenKind::PermanentPolicyViolation { ref reason } => {
                    tracing::error!(
                        "[Claude] Permanent policy violation detected on account {}, marking forbidden: {}",
                        email,
                        reason
                    );
                    if let Err(e) = token_manager.set_forbidden(&account_id, &error_text).await {
                        ...
                        tracing::warn!("[Claude] Account {} permanently marked as forbidden on disk", email);
                    }
                }
                crate::proxy::token_manager::ForbiddenKind::TransientQuarantine { cooldown_secs } => {
                    tracing::warn!(
                        "[Claude] Transient 403 detected on account {}, applying {}s in-memory quarantine without disk invalidation",
                        email,
                        cooldown_secs
                    );
                    token_manager
                        .quarantine_account(&account_id, cooldown_secs, &error_text, false)
                        .await;
                }
            }
        }
