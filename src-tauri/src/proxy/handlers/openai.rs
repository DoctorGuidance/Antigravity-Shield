            ...
            if let Some(acc_id) = token_manager.get_account_id_by_email(&email) {
                ...
                match forbidden_kind {
                    crate::proxy::token_manager::ForbiddenKind::ValidationRequired { cooldown_secs, .. } => {
                        tracing::warn!(
                            "[OpenAI] VALIDATION_REQUIRED detected on account {}, applying {}s cooldown",
                            email,
                            cooldown_secs
                        );
                    ...
                    crate::proxy::token_manager::ForbiddenKind::PermanentPolicyViolation { ref reason } => {
                        tracing::error!(
                            "[OpenAI] Permanent policy violation detected on account {}, marking forbidden: {}",
                            email,
                            reason
                        );
                        if let Err(e) = token_manager.set_forbidden(&acc_id, &error_text).await {
                            ...
                            tracing::warn!("[OpenAI] Account {} permanently marked as forbidden on disk", email);
                        }
                    }
                    crate::proxy::token_manager::ForbiddenKind::TransientQuarantine { cooldown_secs } => {
                        tracing::warn!(
                            "[OpenAI] Transient 403 detected on account {}, applying {}s in-memory quarantine without disk invalidation",
                            email,
                            cooldown_secs
                        );
                        token_manager
