...
pub async fn handle_generate(
    ...
    while let Some(attempt) = next_rotation_attempt(
        ...
        // [HARDENED M1] 403 细分分类与安全隔离：
        // 1. VALIDATION_REQUIRED: 设置 10 分钟临时验证锁与内存隔离，提取 validation_url，不注销账号
        // 2. 永久违规 (Permanent Policy Violation): 仅在明确检测到封号/违规时写入 is_forbidden
        // 3. 其它 403: 实行 5 分钟内存冷却隔离 (in-memory quarantine)，绝不覆写磁盘 disabled=true
        if status_code == 403 {
            if let Some(acc_id) = token_manager.get_account_id_by_email(&email) {
                let forbidden_kind = crate::proxy::token_manager::classify_forbidden_error(&error_text);
                match forbidden_kind {
                    crate::proxy::token_manager::ForbiddenKind::ValidationRequired { cooldown_secs, .. } => {
                        tracing::warn!(
                            "[Gemini] VALIDATION_REQUIRED detected on account {}, applying {}s cooldown",
                            email,
                            cooldown_secs
                        );
                        let block_until = chrono::Utc::now().timestamp() + cooldown_secs as i64;
                        ...
                        token_manager
                            .quarantine_account(&acc_id, cooldown_secs, &error_text, true)
                            .await;
                    }
                    crate::proxy::token_manager::ForbiddenKind::PermanentPolicyViolation { ref reason } => {
                        tracing::error!(
                            "[Gemini] Permanent policy violation detected on account {}, marking forbidden: {}",
                            email,
                            reason
                        );
                        if let Err(e) = token_manager.set_forbidden(&acc_id, &error_text).await {
                            ...
                            tracing::warn!("[Gemini] Account {} permanently marked as forbidden on disk", email);
                        }
                    }
                    crate::proxy::token_manager::ForbiddenKind::TransientQuarantine { cooldown_secs } => {
                        tracing::warn!(
                            "[Gemini] Transient 403 detected on account {}, applying {}s in-memory quarantine without disk invalidation",
                            email,
                            cooldown_secs
                        );
                        token_manager
                            .quarantine_account(&acc_id, cooldown_secs, &error_text, false)
                            .await;
                    }
                }
            }
...
pub async fn handle_list_models(
    ...
    use crate::proxy::common::model_mapping::get_dynamic_models_scoped;

    let target_account_id = params
        .get("account_id")
        ...
        .or_else(|| {
            headers
                .get("x-account-id")
                .or_else(|| headers.get("x-antigravity-account"))
                .and_then(|h| h.to_str().ok())
                .map(|s| s.to_string())
        });

    // 获取动态模型列表（与 /v1/models 一致）
    let only_raw = *state.only_raw_quota_models.read().await;
    let model_ids = get_dynamic_models_scoped(
