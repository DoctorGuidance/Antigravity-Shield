                                        ...
                                        if let Ok(mut json) = serde_json::from_str::<Value>(json_part) {
                                            ...
                                            if let Some(candidates) = actual_data.get("candidates").and_then(|c| c.as_array()) {
                                                ...
                                                for (idx, candidate) in candidates.iter().enumerate() {
                                                    ...
                                                    if let Some(parts_list) = parts {
                                                        for part in parts_list {
                                                            ...
                                                            if let Some(text) = part.get("text").and_then(|t| t.as_str()) {
                                                                ...
                                                                } else if !client_tool_names.is_empty() {
                                                                    // [FIX #3379] 尝试在流式文本中恢复 call:default_api:* 为结构化 tool_calls
                                                                    if let Some((prose, recovered_call)) = super::response::try_recover_call_default_api(&clean_text, &client_tool_names) {
                                                                        if let Some(p) = prose {
                                                                            content_out.push_str(&p);
                                                                        }
                                                                        let func = recovered_call.function.unwrap_or(super::models::ToolFunction {
                                                                            name: "unknown".to_string(),
                                                                            arguments: "{}".to_string(),
                                                                        });
                                                                        let call_key = format!("recovered_{}_{}", func.name, func.arguments);
                                                    ...
                                                    // [FIX #3379] 如果积累的 content_out 整体匹配 call:default_api:*，在发射前进行补救
                                                    if emitted_tool_calls.is_empty() && !content_out.is_empty() && !client_tool_names.is_empty() {
                                                        if let Some((prose, recovered_call)) = super::response::try_recover_call_default_api(&content_out, &client_tool_names) {
                                                            let func = recovered_call.function.unwrap_or(super::models::ToolFunction {
                                                                name: "unknown".to_string(),
                                                                arguments: "{}".to_string(),
                                                            });
                                                            ...
                                                            content_out = prose.unwrap_or_default();
                                                        }
                                                    }
