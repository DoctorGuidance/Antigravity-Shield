...
pub fn parse_loose_json_args(args_str: &str) -> Option<serde_json::Value> {
...
/// [FIX #3379] 恢复 Gemini 3.7 call:default_api:* 伪代码泄漏为标准 OpenAI tool_call
pub fn try_recover_call_default_api(
    text: &str,
    registered_tool_names: &std::collections::HashSet<String>,
) -> Option<(Option<String>, ToolCall)> {
    let trimmed = text.trim();
    const PREFIX: &str = "call:default_api:";

    // Guard against markdown code blocks
    if trimmed.starts_with("```") || trimmed.contains("```call:default_api:") {
        return None;
    }

    let prefix_idx = trimmed.find(PREFIX)?;
    if registered_tool_names.is_empty() {
        return None;
    }

    let prose = if prefix_idx > 0 {
        let p = trimmed[..prefix_idx].trim();
        if !p.is_empty() {
            Some(p.to_string())
        } else {
            None
    ...
    let rest = &trimmed[prefix_idx + PREFIX.len()..];
    let tool_end = rest.find(|c| c == '{' || c == '(').unwrap_or(rest.len());
    let tool_name = rest[..tool_end].trim();
    let mut args_str = rest[tool_end..].trim().to_string();

    if tool_name.is_empty() {
        return None;
    }

    // Normalize parentheses syntax: (key: val) -> {key: val}
    if args_str.starts_with('(') && args_str.ends_with(')') {
        args_str = format!("{{{}}}", &args_str[1..args_str.len() - 1]);
    }

    // Whitelist check (case-insensitive)
    let matched_name = registered_tool_names
        .iter()
        .find(|n| n.eq_ignore_ascii_case(tool_name))?
        .clone();

    // Parse arguments JSON
    let parsed_args = if args_str.is_empty() || args_str == "{}" {
        serde_json::json!({})
    } else if let Ok(val) = serde_json::from_str::<serde_json::Value>(&args_str) {
        if val.is_object() {
            val
        } else {
            return None;
        }
    } else if let Some(val) = parse_loose_json_args(&args_str) {
        val
    } else {
        return None;
    };

    let call_id = format!("{}-3379-recovered-{}", matched_name, uuid::Uuid::new_v4());
...
pub fn transform_openai_response(
    ...
    if let Some(candidates) = raw.get("candidates").and_then(|c| c.as_array()) {
        for (idx, candidate) in candidates.iter().enumerate() {
            ...
            // [FIX #3379] call:default_api:* pseudo-code tool call recovery bridge
            if tool_calls.is_empty() && !content_out.is_empty() && !client_tool_names.is_empty() {
                if let Some((prose, recovered_call)) = try_recover_call_default_api(&content_out, client_tool_names) {
                    tracing::warn!(
                        "[#3379-RECOVERY] OpenAI Response: recovering call:default_api:{} as tool_call",
                        recovered_call.function.as_ref().map(|f| &f.name).unwrap_or(&"unknown".to_string())
                    );
                    if let Some(p) = prose {
                        content_out = p;
                    } else {
                    ...
                    tool_calls.push(recovered_call);
                }
            }
