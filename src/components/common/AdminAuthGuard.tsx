...
export const AdminAuthGuard: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        // 检查 Session 存储 (优先)
        const rawSession = sessionStorage.getItem('abv_admin_auth_token') || sessionStorage.getItem('abv_admin_api_key');
        if (rawSession) {
            const resolved = decryptSensitiveData(rawSession);
        ...
        // 检查本地存储 (迁移逻辑)
        const rawLocal = localStorage.getItem('abv_admin_auth_token') || localStorage.getItem('abv_admin_api_key');
        if (rawLocal) {
            const resolved = decryptSensitiveData(rawLocal);
            sessionStorage.setItem('abv_admin_auth_token', encryptSensitiveData(resolved));
            sessionStorage.removeItem('abv_admin_api_key');
            localStorage.removeItem('abv_admin_auth_token');
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            // 安全存储 token
            sessionStorage.setItem('abv_admin_auth_token', encryptSensitiveData(trimmedKey));
            sessionStorage.removeItem('abv_admin_api_key');
